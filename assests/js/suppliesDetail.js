...
function renderPrimaryDetails(product) {
  ...
  const images = product.images && product.images.length > 0 ? product.images : ["./assests/img/products/ReplaceImage.webp"];
      ...
      .map(
        (img, index) => `
              <img
                ...
                data-full="${img}"
                onerror="this.onerror=null; this.src='./assests/img/products/ReplaceImage.webp';"
              />
            `
      )
      .join("")}
  ...
  producter.innerHTML = `
    <div class="image-gallery" style="flex: 1; min-width: 300px; display: flex; flex-direction: row; gap: 15px; border: none; padding: 0; margin: 0; align-self: flex-start;">
      ...
      <div class="main-image-container" style="flex-grow: 1; border: 1px solid #e1e1e1; background: #fff; padding: 20px; border-radius: 8px; max-height: 400px; overflow: hidden; display: flex; justify-content: center; align-items: center;">
        <img id="main-image" src="${images[0]}" alt="${product.title} Image" style="max-height: 100%; max-width: 100%; object-fit: contain; transition: transform 0.1s ease-out;" onerror="this.onerror=null; this.src='./assests/img/products/ReplaceImage.webp';" />
      </div>
    </div>
